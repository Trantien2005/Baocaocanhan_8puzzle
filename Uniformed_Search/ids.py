...
def depth_limited_search(state, path, limit, visited):
    """
    Tìm kiếm DFS với giới hạn độ sâu.
    Args:
    ...
    Returns:
        list[tuple] or None: Đường đi đến mục tiêu hoặc None nếu không tìm thấy.
    """
    if state == GOAL_STATE:
        return path
    if limit == 0:
    ...
    for child in get_children(state):
        if child not in visited:
            result = depth_limited_search(child, path + [child], limit - 1, visited)
            if result:
                return result
...
def ids(initial_state):
    """
    Tìm kiếm sâu dần bằng cách tăng giới hạn độ sâu.
    Args:
    ...
    while True:
        visited = set()
        result = depth_limited_search(initial_state, [initial_state], depth, visited)
        if result:
            return result
        depth += 1